...
async def get_location_analysis(
    ...
    try:
        ...
        location_data = None
        if db:
            collection = db.get_collection("location_analytics")
            if collection:
                location_data = await collection.find_one({"location": location})
        ...
        if not location_data:
            ...
            if len(features_df) > 0:
                ...
                # Generate insights
                insights = [
                    f"Properties in {location} are in high demand.",
                ...
                location_data = {
                    "location": location,
                    "district": district,
                    "average_price": 350000,  # Placeholder
...
async def get_price_forecast(
    ...
    try:
        # Define default results when DB is not available
        time_series = []
        
        # Get current date for generating synthetic data
        current_date = datetime.now()
        base_price = 350000  # Default base price
        
        # Generate synthetic data points for past 12 months
        for i in range(1, 13):
            month_date = (current_date.replace(day=1) - pd.DateOffset(months=i)).strftime("%Y-%m")
            import random
        ...
        if time_series:
            ...
            month_date = datetime.strptime(time_series[-1]["date"], "%Y-%m")
